...
PyObject *py_ue_set_physics_angular_velocity(ue_PyUObject * self, PyObject * args)
{
	...
	primitive->SetPhysicsAngularVelocityInRadians(new_ang_vel, add_to_current, f_bone_name);
	...
	if (!destructible)
	{
		...
		else
		{
			...
			if (component)
			{
				...
				destructible = (UDestructibleComponent *)actor->GetComponentByClass(UDestructibleComponent::StaticClass());
#else
				for (UActorComponent *outerComponent : actor->GetComponents())
				{
					if (Cast<IDestructibleInterface>(outerComponent))
					{
						destructible = (IDestructibleInterface *)outerComponent;
						break;
					}
				}
#endif
